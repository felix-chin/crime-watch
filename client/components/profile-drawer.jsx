...
export default class ProfileDrawer extends React.Component {
  ...
  render() {
    ...
    return (
      <>
        <i onClick={this.toggleMenu} className='fas fa-bars menu-icon ml-3 mt-3 pointer' style={{ zIndex: 1 }}></i>
        { isOpen &&
          <div onClick={this.toggleMenu} className='shade animate'></div>
        }
        <div className='d-flex flex-column justify-content-end align-items-center menu roboto-font text-white pt-3 pb-2' style={isOpen ? openedMenu : closedMenu}>
          <h3 onClick={() => setView('profile', {})} className="pb-1 pointer">Profile</h3>
